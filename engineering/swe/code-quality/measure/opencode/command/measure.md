...
Locate the code-quality:measure skill's own directory — it contains `assets/rust/run.sh` under
a `code-quality/measure` path (check `.opencode/skills/`, `~/.config/opencode/skills/`,
`.claude/skills/measure`, or `~/.claude/skills/measure`) — and run:

    assets/run.sh ${ARGUMENTS:-.}
...
function, line, score, and threshold. Only report findings it scores at confidence 80 or above
— it's instructed to default to skepticism, so a low score means treat the finding as a false
positive or acceptable complexity, not an error worth surfacing.
