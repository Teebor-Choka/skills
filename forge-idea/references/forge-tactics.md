...
is not, because reality is not polite. But the aggression is aimed at the *branch*,
...
3. **True but already owned** — holds but an incumbent / open-source project /
   standards body / vendor already owns it. → adapt by finding the seam they *don't*
...
- **A crowded field is demand, not a wall.** Fifteen organizations doing something
  ...
  ask where it is *underserved*, not whether to enter.
...
- **Timing / regulatory** — verify the exact date, scope, and whether a cited
  regulation actually mandates *this* mechanism (regulations mandate outcomes, not
  your solution).
...
opinion. When they split, the split *is* the finding — surface it, don't average it.
